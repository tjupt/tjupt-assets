...
(function () {
    ...
    var DS = {
        inquiry: function (data, str, num) {
            ...
            $.post('at_user.php', {'str': str}, phpdata => {
    ...
    var AutoTips = function (A) {
        ...
        _this.start = function () {
            if (!D.$(config.boxID)) {
                ...
                var div = D.DC('DIV');
                var bs = D.BS();
                h = h.replace('$top$', (info.top + bs.top)).replace('$left$', (info.left + bs.left)).replace('$width$', info.width).replace('$height$', info.height).replace('$SCTOP$', '0');
                div.innerHTML = h;
